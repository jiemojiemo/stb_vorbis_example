...
bool AudioFile<T>::decodeWaveFile (std::vector<uint8_t>& fileData)
{
    ...
    fourBytesToInt (fileData, f + 4);  // int32_t formatChunkSize = fourBytesToInt (fileData, f + 4);
    ...
    for (int i = 0; i < numSamples; i++)
    {
        for (int channel = 0; channel < numChannels; channel++)
        {
            ...
            {
                int32_t sampleAsInt = fourBytesToInt(fileData, sampleIndex, Endianness::LittleEndian);
                T sample = (T)sampleAsInt/(T)(std::numeric_limits<std::int32_t>::max());
                samples[channel].push_back(sample);
            }
            else if(bitDepth == 32 && audioFormat == 3)
            {
                float sample = fourBytesToFloat(fileData, sampleIndex, Endianness::BigEndian);
                samples[channel].push_back(sample);
            }
            else
            {
...
bool AudioFile<T>::save (std::string filePath, AudioFileFormat format)
{
    ...
    bool success = encode(fileData, format);
    return success && writeDataToFile(fileData, filePath);
}

//=============================================================
template <class T>
bool AudioFile<T>::encode(std::vector<uint8_t> &fileData, AudioFileFormat format)
{
    if (format == AudioFileFormat::Wave)
    {
        return encodeToWaveFile(fileData);
    }
    else if (format == AudioFileFormat::Aiff)
    {
        return encodeToAiffFile(fileData);
    }
...
//=============================================================
template <class T>
bool AudioFile<T>::encodeToWaveFile(std::vector<uint8_t> &fileData)
{
    ...
    addInt16ToFileData (fileData, audioFormat); // audio format = 1 or 3
    ...
    for (int i = 0; i < getNumSamplesPerChannel(); i++)
    {
        for (int channel = 0; channel < getNumChannels(); channel++)
        {
            ...
            {
                int32_t sampleAsIntAgain = (int32_t) (samples[channel][i] * std::numeric_limits<int32_t>::max());
            ...
            {
                uint8_t bytes[4];
                *reinterpret_cast<float*>(&bytes[0]) = samples[channel][i];
    ...
    {
        std::cout << "ERROR: couldn't encode to wave audio" << std::endl;
        return false;
    }
...
//=============================================================
template <class T>
bool AudioFile<T>::encodeToAiffFile(std::vector<uint8_t> &fileData)
{
    ...
    {
        std::cout << "ERROR: couldn't encode to aiff audio" << std::endl;
        return false;
    }
...
{
    float result = 0.0f;
    std::memcpy(&result, &source[startIndex], sizeof(float));
...
template<class T>
const T *AudioFile<T>::getReadPointer(int channelNumber) const {
    return samples[channelNumber].data();
}

template<class T>
T *AudioFile<T>::getWritePointer(int channelNumber) {
    return samples[channelNumber].data();
}
